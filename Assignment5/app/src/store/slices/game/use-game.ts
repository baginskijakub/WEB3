...
import { getWebSocket } from './game-middleware'; // Adjust the import path as needed
...
export const useGame = () => {
    ...
    const playCard = useCallback((cardIndex: number, requestColor?: Color) => {
        getWebSocket()?.send(JSON.stringify({ type: 'PLAY_CARD', payload: { cardIndex, requestColor } }));
    }, []);

    const drawCard = useCallback(() => {
        getWebSocket()?.send(JSON.stringify({ type: 'DRAW_CARD' }));
    }, []);

    const sayUno = useCallback(() => {
        const playerIndex = gameState?.players.findIndex((p) => p === user?.name);
        getWebSocket()?.send(JSON.stringify({ type: 'SAY_UNO', payload: { playerIndex } }));
    ...
    const accuseOfNotSayingUno = useCallback((accused: number) => {
        ...
        getWebSocket()?.send(JSON.stringify({ type: 'ACCUSE_OF_NOT_SAYING_UNO', payload: { accuser, accused } }));
    }, [gameState?.players, user?.name]);
