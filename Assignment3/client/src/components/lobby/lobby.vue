...
<template>
  <div class="w-80 p-4 flex flex-col gap-2 bg-white rounded-lg">
    ...
    <div v-for="p in props.lobby.players" class="text-sm p-2 bg-slate-200 rounded-md flex justify-between items-center">
      <label for="opponent">{{ p.name }}</label>
    </div>

    <button v-if="!hasJoined" class="px-2 py-1 bg-blue-200 text-blue-700 rounded-md w-full" @click="handleJoinLobby()">
      Join
    </button>
    <button
